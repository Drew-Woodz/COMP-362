...
int main(int argc, char** argv)
{
	...
	if (cifsCreateFileSystem(argv[1]) != CIFS_NO_ERROR)
		exit(EXIT_FAILURE);

	if (cifsMountFileSystem(argv[1]) != CIFS_NO_ERROR)
		exit(EXIT_FAILURE);
	...
	if (cifsUmountFileSystem(argv[1]) != CIFS_NO_ERROR)
		exit(EXIT_FAILURE);
...
void testStep1()
{
    ...
    printf("TESTING CREATING A FILE WITHOUT AN OPEN WORKING DIRECTORY: ");
    if(cifsCreateFile("test", CIFS_FILE_CONTENT_TYPE) == CIFS_OPEN_ERROR){
        printf("PASS\n");
    }
    else{
        printf("FAIL\n");
    ...
    printf("TESTING TO SEE IF THERE IS A DUPLICATE FILE: ");
    if(cifsCreateFile("test", CIFS_FOLDER_CONTENT_TYPE) == CIFS_DUPLICATE_ERROR){
    ...
    printf("TESTING TO SEE IF FILE WAS CREATED: ");
    if(cifsCreateFile("test", CIFS_FILE_CONTENT_TYPE) == CIFS_ALLOC_ERROR){
