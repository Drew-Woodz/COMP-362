...
CIFS_ERROR cifsCreateFileSystem(char* cifsFileName)
{
	...
	// initialize the block holding the root folders; there are two of them: folder descriptor and the index block
...
CIFS_ERROR cifsMountFileSystem(char* cifsFileName)
{
	...
	//##############################################################################################################################//
	//read the bitvector into mem
	/*
	int b = 0;//bit
	for(size_t i = 0; i < CIFS_SUPERBLOCK_INDEX; i++){
		blockBuffer = cifsReadBlock(i);
		for(size_t j = 0; j < CIFS_BLOCK_SIZE; j++){
			cifsContext->bitvector[b] = blockBuffer[j];
			b++;
		}
	...
	*/
	...
	//struct cifs_block_type *block = calloc(1, sizeof(CIFS_BLOCK_TYPE));

	//blockBuffer = cifsReadBlock(cifsContext-superblock->cifsRootNodeIndex);
	//block = (struct cifs_block_type*) blockBuffer;

	//Add the entry to the hash table
	/*
	unsigned long directoryIndex = hash(cifsFileName);
	CIFS_REGISTRY *registry = malloc(CIFS_REGISTRY_SIZE);
		(*cifsContext->registry)[directoryIndex] = registry;
	*/	
	return CIFS_NO_ERROR;
}
...
CIFS_ERROR getCurrentWorkingDirectory(CIFS_FILE_HANDLE_TYPE *fileHandleType)
{
    CIFS_PROCESS_CONTROL_BLOCK_TYPE *ctrlBlock = cifsContext->processList;
    bool matchDir = false;
    
    while(!matchDir && ctrlBlock != NULL){
       //If there is no working directory return open error
       if(ctrlBlock->openFiles != NULL){
           //If found assign it to the filehandle
           *fileHandleType = ctrlBlock->openFiles->fileHandle;
           matchDir = true;
       }
       else{
           ctrlBlock = ctrlBlock->next;
       }
    }
...
CIFS_ERROR cifsCreateFile(CIFS_NAME_TYPE filePath, CIFS_CONTENT_TYPE type)
{
	...
	bool processFinder = false;

	CIFS_PROCESS_CONTROL_BLOCK_TYPE *sameProcess = cifsContext->processList;

	while(sameProcess != NULL){
		if(sameProcess->pid == getpid()){
			processFinder = true;
		}
		else{
			processFinder = sameProcess->next;
		}
	}
	...
	//No matching process found or the match has no openFile
	if(!processFinder || sameProcess == NULL || sameProcess->openFiles == NULL){
		return CIFS_OPEN_ERROR;
	}
	//The file name already exists in the directory
	CIFS_FILE_DESCRIPTOR_TYPE infoBuffer;
	if(cifsGetFileInfo(filePath, &infoBuffer) == CIFS_NO_ERROR){
		return CIFS_DUPLICATE_ERROR;
	}
	//Otherwise, create the file
	CIFS_BLOCK_TYPE *createBlock = malloc(sizeof(CIFS_BLOCK_TYPE));
	if(createBlock == NULL){
		return CIFS_ALLOC_ERROR;
	}
	createBlock-> type = type;

	//set file identifier to the value of the next ident from superbloc
	createBlock->content.fileDescriptor.identifier = cifsContext->superblock->cifsNextUniqueIdentifier;
	//increment to next abailable superblock value
	(cifsContext->superblock->cifsNextUniqueIdentifier)++;

	//
	//##############################################################################################################################//

	//find available block in storage using in-memory bitvector
	//flips the bit indicating block is taken
	...
	CIFS_REGISTRY_ENTRY_TYPE *createEntry = malloc(sizeof(CIFS_REGISTRY_ENTRY_TYPE));
	...
	clock_gettime(CLOCK_MONOTONIC, &createTime);

	strcpy(createEntry->fileDescriptor.name, filePath);

	createEntry->fileDescriptor.creationTime = createTime.tv_sec;
	createEntry->fileDescriptor.lastAccessTime = createTime.tv_sec;
	createEntry->fileDescriptor.lastModificationTime = createTime.tv_sec;
	createEntry->fileDescriptor.owner = cifsContext->registry;
	createEntry->fileDescriptor.size = 0;
	createEntry->fileDescriptor.block_ref =  freeBlock;
	createEntry->fileDescriptor.parent_block_ref = sameProcess->openFiles->fileHandle;

	createBlock->content.fileDescriptor.accessRights = umask(cifsContext->registry);

	createEntry->parentFileHandle = sameProcess->openFiles->fileHandle;

	createEntry->referenceCount = 0;

	long hashedFileName = hash(filePath);
	
	createEntry->next = (*cifsContext->registry)[hashedFileName];
	(*cifsContext->registry)[hashedFileName] = createEntry;

	//##############################################################################################################################//
	//

	//Copy the local file descriptor to the disk block that was found to be free (the on that will hold the on-volume file descriptor)
	memcpy(&cifsVolume->_fileno[&hashedFileName], &createBlock, sizeof(*createBlock));
 ...
 *
 * Hashes the file name and check if the file is in the registry. If not, then it returns CIFS_NOT_FOUND_ERROR.
 *
 * If the file is found, but the reference count is not zero (i.e., at least one process has the file opened),
 * then the function returns CIFS_IN_USE_ERROR.
 *
 * Otherwise:
 *    - if it is a folder that is not empty, then returns CIFS_NOT_EMPTY_ERROR.
 *    - Otherwise:
 *       - checks if the process owner can delete this file or folder; if not, it returns CIFS_ACCESS_ERROR.
 *       - Otherwise:
 ...
 *          - clears the entry in the parent folder for this file in the in-memory registry
 *          - copies the relevant block of the in-memory bitvector to the corresponding bitevector blocks on the volume
 *          - copies the in-memory file descriptor to the volume
 *
 */
CIFS_ERROR cifsDeleteFile(CIFS_NAME_TYPE filePath)
{
	...
	CIFS_BLOCK_TYPE *descriptor = (CIFS_BLOCK_TYPE *) &(cifsVolume->_fileno);
	if(descriptor->type == CIFS_FOLDER_CONTENT_TYPE){
		if(descriptor->content.fileDescriptor.size != 0){
			return CIFS_NOT_EMPTY_ERROR;
		}
	}
	...
	//Checks that the process owner can delete the file or folder
	//set the cwd and cwdfd
	CIFS_FILE_HANDLE_TYPE currentWorkingDirectory;
	if(getCurrentWorkingDirectory(&currentWorkingDirectory) != CIFS_NO_ERROR){
		return CIFS_OPEN_ERROR;
	}
...
CIFS_ERROR cifsOpenFile(CIFS_NAME_TYPE filePath, mode_t desiredAccessRights, CIFS_FILE_HANDLE_TYPE *fileHandle)
{
    ...
    //If the file is not found in the in-memory registry, then the function returns CIFS_NOT_FOUND_ERROR (since only
    //existing files can be opened).
	if(cifsGetFileInfo(filePath, NULL) == CIFS_NOT_FOUND_ERROR){
		return CIFS_NOT_FOUND_ERROR;
	}
    //Checks if the file is already open by any process by examining the list of processes in the context.
 	//If so, then the function returns CIFS_OPEN_ERROR.
	CIFS_PROCESS_CONTROL_BLOCK_TYPE *openProcess = cifsContext->processList;

	unsigned long openID = 0;

	while(openProcess != NULL){
		if(openProcess->pid == getpid()){
			if(openProcess->openFiles->identifier == openID){
				return CIFS_OPEN_ERROR;
			}
		}
		openProcess = openProcess->next;
	}
...
CIFS_ERROR cifsGetFileInfo(CIFS_NAME_TYPE filePath, CIFS_FILE_DESCRIPTOR_TYPE* infoBuffer)
{
	...
	//
	CIFS_FILE_HANDLE_TYPE CurrentWorkingDirectory;
	if(getCurrentWorkingDirectory(&getCurrentWorkingDirectory) != CIFS_NO_ERROR){
		return CIFS_OPEN_ERROR;
	}

	unsigned long hashFilePath = hash(filePath);
	CIFS_REGISTRY_ENTRY_TYPE *filePathRegistry = (*cifsContext->registry) [hashFilePath];

	while(filePathRegistry != NULL){
		if(strcmp(filePath, filePathRegistry->fileDescriptor.name) == 0){
			memcpy(infoBuffer, &filePathRegistry, sizeof(CIFS_FILE_DESCRIPTOR_TYPE));
			return CIFS_NO_ERROR;
		}
		filePathRegistry = filePathRegistry->next;
	}
...
CIFS_ERROR cifsWriteFile(CIFS_FILE_HANDLE_TYPE fileHandle, char* writeBuffer)
{
	...
	//##############################################################################################################################//
	//
	CIFS_PROCESS_CONTROL_BLOCK_TYPE *controlBlocks = cifsContext->processList;

	bool processFinder = false;
	while(controlBlocks != NULL){
		if(controlBlocks->pid ==getpid()){
			processFinder = true;
		}
		else{
			controlBlocks = controlBlocks->next;
		}
		controlBlocks->openFiles[fileHandle].fileHandle = CIFS_INVALID_INDEX;

		if(controlBlocks->openFiles[fileHandle].fileHandle){
			return CIFS_NOT_FOUND_ERROR;
		}
	}
